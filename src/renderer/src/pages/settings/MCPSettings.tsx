...
const MCPSettings: FC = () => {
  ...
  const installDependencies = async () => {
    try {
      ...
      window.message.success(t('settings.mcp.installSuccess'))
      setIsInstalling(false)
    } catch (error: any) {
      window.message.error(`${t('settings.mcp.installError')}: ${error.message}`)
      setIsInstalling(false)
      // Refresh the status after error
      checkBinaries()
    }
  }
      ...
      <SettingGroup theme={theme}>
        ...
        {!isUvInstalled || !isBunInstalled ? (
          <div style={{ marginBottom: 20 }}>
            <Alert
              type="info"
              message={t('settings.mcp.title')}
              description={
                <div>
                  <p>{t('settings.mcp.config_description')}</p>
                  <p>
                    {isUvInstalled ? '' : `UV ${t('settings.mcp.missingDependencies')}`}
                    <br />
                    {isBunInstalled ? '' : `Bun ${t('settings.mcp.missingDependencies')}`}
                  </p>
                </div>
              }
              showIcon
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
              <Button
                ...
                size="large">
                {isInstalling ? t('settings.mcp.dependenciesInstalling') : t('settings.mcp.installDependencies')}
              </Button>
            </div>
            ...
            {isInstalling && (
              <div style={{ textAlign: 'center', marginTop: 16 }}>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text type="secondary">
                    {(!isUvInstalled || !isBunInstalled) ?? t('settings.mcp.dependenciesInstalling')}
                  </Text>
                </div>
              </div>
            )}
